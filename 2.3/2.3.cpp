...
void RenderScene(void)
{
	...
	GLint n = 11;
	GLTVector3 vNormal;
	GLTVector3 vCorners[32] =
	{
		{ 0.0f, 1.0f, 0.0f }, // G鏎a 0
	};
	...
	for (GLint i = 1; i <= n; i++)
	{
		...
		vCorners[i][0] = vCorners[i - 1][0] + ((0.5*cos(i * 2 * 3.14f / n)) * 0.7f);
		vCorners[i][2] = vCorners[i - 1][2] + ((0.5*sin(i * 2 * 3.14f / n)) * 0.7f);
	...
	// Czyszczenie okna aktualnym kolorem czyszcz鉍ym
	glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
	// Zapisanie stanu macierzy i wykonanie obrot闚
	glPushMatrix();
	// Cofni璚ie obiekt闚
